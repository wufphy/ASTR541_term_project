...
def plotRotcurve(Pot,*args,**kwargs):
    ...
    INPUT:
       ...
       savefilenameRC= save the rotation curve data to this file, added by Feng Wu on December 2, 2020
    ...
    #output RC data
    savefilenameRC= kwargs.pop('savefilenameRC',None)
    #filenameRC=savefilenameRC+'.txt'
    fileoutrc = open(savefilenameRC, 'w')     # file with rotation curve measurements
    nrc=len(rotcurve)
    for elem1 in range(0,nrc):
        fileoutrc.write(str(Rs[elem1]))
        fileoutrc.write('      ')
        fileoutrc.write(str(rotcurve[elem1]))
        fileoutrc.write('\n')
    fileoutrc.close()
    print("rotation curve data: "+savefilenameRC)
